...
from homeassistant.const import (CONF_NAME, CONF_ID, CONF_LIGHTS, CONF_SOCKETS)
#import validation
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    sockets = config[CONF_SOCKETS]
    for socket in sockets:
        deviceid = socket[CONF_ID]
        name = socket[CONF_NAME]
        dflink = socket[CONF_RFLINK]
        device = LRFSocket(name, DEFAULT_STATE, deviceid, rflink, rabbithost, rabbitport, rabbitque, rabbituname, rabbitpass)
        devices.append(device)    
